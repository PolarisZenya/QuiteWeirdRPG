...
/*:zh_TW
* @target MZ
* @plugindesc 官方通用參數型態轉換工具
* @author トリアコンタン
*
* @help PluginCommonBase.js
*
* 此插件是基本插件，應該由其他插件引用。
* 安裝此插件後，可以在各種情況下使用以下控製字符。
*
* 可用的控製字符
*  \v[n]：轉換為第n個變量的值。
*  \s[n]：轉換為第n個開關值（true,false）。
*  \ss[n]：轉換為自切換值（true,false）
* n-> A，B，C，D
*
* 可以使用控製字符的情況如下。
*  ・句子顯示
...
*  ・插件參數（*）
*  ・技能等的說明欄
*  *僅基於PluginCommonBase.js導入的插件
*
*以下是針對開發人員的評論。
*
*通過將此插件作為基礎，您可以分析參數和
*檢索插件命令和備註字段時自動執行參數類型和控製字符轉換。
*通過標準化控製字符轉換處理，與所有從屬插件一致
*實現控製字符轉換規範。
*
* ・插件參數的類型分析
*自動類型分析和轉換插件參數。
*另外，獲取參數時將自動轉換控製字符。
*調用以下方法獲取返回值。
*確保指定“ document.currentScript”作為參數。
*
* PluginManagerEx.createParameter（document.currentScript）;
*
*但是，如果參數名稱的值小於或等於結尾（大小寫）
*不執行控製字符或類型轉換，並且按原樣返回值。
* 文本
* 名稱
* 筆記
*說明
...
*自動類型分析和轉換插件命令參數。
*另外，獲取參數時將自動轉換控製字符。
*使用以下內容代替PluginManager.registerCommand。
*確保指定“ document.currentScript”作為參數。
*
* PluginManagerEx.registerCommand（document.currentScript，“ command”，args => {
*}）;
...
* ・執行動態共同事件
*您可以執行動態常見事件。
*動態公共事件是堆棧類型的並行處理公共事件。
*您可以並行執行多個具有相同ID的常見事件。
*它會自動從已執行的操作中銷毀。
*
* $gameMap.setupDynamicCommon(commonEventId);
*/
