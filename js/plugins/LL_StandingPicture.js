//=============================================================================
// RPGツクールMZ - LL_StandingPicture.js v2.5.1
//-----------------------------------------------------------------------------
// ルルの教会 (Lulu's Church)
 ...
 *   これにより変数の状態で立ち絵を変化させるといったこともできます。
 * ・立ち絵を同時に4人まで表示することが可能になりました。
 *
 * 専用制御文字:
...
(() => {
	...
	let animationCount3 = 0;
	let spriteSPicture3 = null;
	let showSPicture3 = false;
	let refSPicture3 = false;
	let motionSPicture3 = "";

	let animationCount4 = 0;
	let spriteSPicture4 = null;
	let showSPicture4 = false;
	let refSPicture4 = false;
	let motionSPicture4 = "";
	...
	class ExStandingPicture {
		...
		static update (elm) {
			...
			// 立ち絵3ピクチャ作成
			if (spriteSPicture3 && refSPicture3) {
				this.refresh(elm._spSprite3, spriteSPicture3, 3);
				refSPicture3 = false;
			}
			// 立ち絵4ピクチャ作成
			if (spriteSPicture4 && refSPicture4) {
				this.refresh(elm._spSprite4, spriteSPicture4, 4);
				refSPicture4 = false;
			}

			// フォーカス処理
			if (focusSPicture !== null) {
				if (focusSPicture != 1) elm._spSprite.setBlendColor([0, 0, 0, (focusToneAdjust * -1)]);
				if (focusSPicture != 2) elm._spSprite2.setBlendColor([0, 0, 0, (focusToneAdjust * -1)]);
				if (focusSPicture != 3) elm._spSprite3.setBlendColor([0, 0, 0, (focusToneAdjust * -1)]);
				if (focusSPicture != 4) elm._spSprite4.setBlendColor([0, 0, 0, (focusToneAdjust * -1)]);
			}
			...
			if (showSPicture3) {
				this.fadeIn(elm._spSprite3, spriteSPicture3, 3);
			} else {
				this.fadeOut(elm._spSprite3, spriteSPicture3, 3);
			}
			if (showSPicture4) {
				this.fadeIn(elm._spSprite4, spriteSPicture4, 4);
			} else {
				this.fadeOut(elm._spSprite4, spriteSPicture4, 4);
			}
			...
			if (!elm._spSprite3.opening && !elm._spSprite3.closing && animationCount3 > 0) {
				animationCount3 = this.animation(elm._spSprite3, motionSPicture3, animationCount3);
			}
			if (!elm._spSprite4.opening && !elm._spSprite4.closing && animationCount4 > 0) {
				animationCount4 = this.animation(elm._spSprite4, motionSPicture4, animationCount4);
			}
	...
	Window_Message.prototype.startMessage = function() {
		...
		processEscapeNumber = messageAllText.match(/\\FFF\[(\w+)\]/);
		if (processEscapeNumber) {
			if (processEscapeNumber[1]) {
		...
		processEscapeNumber = messageAllText.match(/\\FFFF\[(\w+)\]/);
		if (processEscapeNumber) {
			if (processEscapeNumber[1]) {
		...
		processEscapeNumber = messageAllText.match(/\\MMM\[(\w+)\]/);
		if (processEscapeNumber) {
			if (processEscapeNumber[1]) {
		...
		processEscapeNumber = messageAllText.match(/\\MMMM\[(\w+)\]/);
		if (processEscapeNumber) {
			if (processEscapeNumber[1]) {
		...
		if (processEscapeNumber) {
			if (processEscapeNumber[1]) {
				...
				if (focusSPicture == "FFF") focusSPicture = 3;
				if (focusSPicture == "FFFF") focusSPicture = 4;
		...
		if (sPictureNumber) {
			...
			animationCount = ANIMATION_FRAME[motionSPicture];
		} else {
			showSPicture = false;
			motionSPicture = "";
		}
		...
		if (sPictureNumber2) {
			...
			animationCount2 = ANIMATION_FRAME[motionSPicture2];
		} else {
			showSPicture2 = false;
			motionSPicture2 = "";
		}
		...
		if (sPictureNumber3) {
			...
			// 再生モーション定義
			motionSPicture3 = sPictureMotion3 ? sPictureMotion3 : "none";
			animationCount3 = ANIMATION_FRAME[motionSPicture3];
		} else {
			showSPicture3 = false;
			motionSPicture3 = "";
		}
		...
		if (sPictureNumber4) {
			...
			// 再生モーション定義
			motionSPicture4 = sPictureMotion4 ? sPictureMotion4 : "none";
			animationCount4 = ANIMATION_FRAME[motionSPicture4];
		} else {
			showSPicture4 = false;
			motionSPicture4 = "";
		}
