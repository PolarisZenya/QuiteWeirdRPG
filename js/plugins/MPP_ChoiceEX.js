 ...
 *
 * @help [version 4.2.0]
 * 該插件適用於RPG Maker MV和MZ。
 * 
 * ▼ 插件命令
 *  - 在MV中，變量N通過在輸入數值的項目中寫入v[N]來引用。
 *  - 在MZ中，通過選擇文本並輸入v[N]可以輸入數值。
 *    指變量N。
 *  - 從[Show Choices]開始執行插件命令基本上是更可取的。
 *  - 如果要在顯示消息窗口的同時使用plugin命令，
 *    請在[顯示文字]之前執行。 
 *  
 *  〇 MV / MZ
 *  
 *  〇 ChoicePos x y row  / 選擇位置設定
 *       x   : 選擇窗口的X坐標
 *       y   : 選擇窗口的Y坐標
 *       row : 選擇窗口中的行數。 如果未設置，則顯示所有
 *   - 指定要顯示的下一個選項的位置（x，y）和行數（行）。
 * 
 *  〇 ChoiceVariableId varId  / 光標位置設定
 *       varId : 可變數
 *   - 設置要顯示的下一個選項的默認位置為變量的值。
 *   - 另外，將當前光標位置放在變量中。
 *   - 至於光標位置，第一個選項是從頂部開始的0到5，下一個選項是10到15，依此類推。
 *     每個選項+10。
 * 
 ...
 *   
 *  〇 ChoiceInMessage  / 在消息中顯示
 *   - 要顯示的下一個選擇顯示在消息窗口中。
 *   - 除非與[顯示文字]結合使用，否則此功能將無法使用。
 *   - 請在[顯示文字]之前執行。
 *   
 * ▼ 更多選項
 *  -  如果連續放置事件命令『顯示選擇』
 *     合併為一個選項。
 *  -  如果您不想將它們放在一起，請在它們之間放一個便條。
 *     可以照常分開。
 *  -  對於『默認』過程，將應用所有選項。
 *  -  對於『取消』過程，將應用除禁止之外的過程。
 *  -  在兩種情況下，以下選項的設置均具有優先權。
 *  -  對於『背景”和“窗口位置』，將應用以下選項的設置。
 * 
 * ▼ 設置顯示項目的條件
 *  - 在選擇的文字中
 *      if（條件）
 *    如果條件為假，則不會顯示該項目。
 *  - 『默認』不顯示該項目會怎樣？
 *    可以在插件參數的[Disabled Index]中設置。
 *  - 『取消』如果未顯示的項目，則執行與禁止相同的處理。
 *  - 在此條件下，s可以引用開關，v可以引用變量。
 *     例：if(s[1]) 的情況
 *          => 1號開關顯示為ON，不顯示顯示為OFF。
 *        if(!s[2]) 的情況
 *          => 開關2處於OFF時顯示，而ON則隱藏。
 *        if(v[5]>0) 的情況
 *          => 如果變量5大於0，則顯示；如果變量5小於0，則隱藏。
 *     
 *     可以在變量中使用的不等式
 *       === : 平等的
 *       !== : 不相等
 ...
 *  @command choiceInMessage
 *      @text 在消息中顯示
 *      @desc 要顯示的下一個選擇顯示在消息窗口中。
 *      除非與[顯示文本]結合使用，否則它將不起作用。
 * 
 * 
 *  @param Max Page Row
 *      @text 顯示的最大行數
